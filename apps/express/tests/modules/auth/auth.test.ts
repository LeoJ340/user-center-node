...
test("password", async () => {
    ...
    assert.ok(await verifyPassword(plainPassword, hashedPassword), "Correct password should pass verification");
    assert.equal(await verifyPassword(wrongPassword, hashedPassword), false, "Wrong password should fail verification");
})
